...
X_train_random = random_split['X_train_split']
X_test_random = random_split['X_test_split']
...
y = np.asarray(random_split['y_train'], dtype=float)
y_test = np.asarray(random_split['y_test'], dtype=float)
...
#============== SAVE MODELS =====================#
model.save(MODELS_DIR/"e2_TL_rand_split.keras", include_optimizer=False)

#============== PREDICTIONS  =====================#

ann_random_split = {}
ann_random_split['x_space'] = X_all
...
ann_random_split['y_test_pred'] = test_pred
ann_random_split['all_space_pred'] = all_pred

with open(PREDICTIONS_DIR / "ann_random_split.pkl", "wb") as f:
    pickle.dump(ann_random_split, f)
